            ...
            <div class="container">
                        ...
                        <div class="gallery-grid">
                            @foreach($attraction->images as $image)
                                <div class="gallery-item">
                                    <img src="{{ asset('storage/'.$image->image) }}" alt="{{ $attraction->title }}">
                                </div>
                            @endforeach
                        </div>
                ...
                <div class="attraction-info">
                    ...
                    <div class="working-hours">
                        <h3>Время работы:</h3>
                        @if($attraction->begin && $attraction->end)
                            <p>{{ $attraction->begin->format('H:i') }} - {{ $attraction->end->format('H:i') }}</p>
                        @else
                            <p>Круглосуточно</p>
                        @endif
                    </div>
                    ...
                    <div class="address">
                        <h3>Адрес:</h3>
                        <p>{{ $attraction->address ?? 'Адрес отсутствует' }}</p>
                    </div>
                    ...
                    <div class="rating-section">
                        ...
                        <div class="average-rating">
                            @include('partials.rating-stars', ['rating' => $attraction->reviews_avg_rating])
                            <span class="rating-value">{{ number_format($attraction->reviews_avg_rating, 1) }}/5</span>
                        </div>
                        <p>На основе {{ $attraction->reviews_count ?? 0 }} отзывов</p>
                    </div>
                </div>
                ...
                <div class="reviews-section">
                    ...
                    <div class="review-form">
                            <form id="reviewForm" action="{{ route('attractions.review') }}" method="POST">
                                @csrf
